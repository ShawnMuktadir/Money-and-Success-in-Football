...
# Crosshair data — separate Leicester for distinct styling
crosshair_df <- label_df28 |>
  mutate(is_leicester = grepl("Leicester", Team))
cross_other  <- crosshair_df |> filter(!is_leicester)
cross_leic   <- crosshair_df |> filter(is_leicester)
fr_labels    <- crosshair_df |> distinct(Financial_Rank) |> arrange(Financial_Rank)
...
p33 <- ggplot() +
  ...
  # Crosshairs — other story clubs
  geom_segment(data = cross_other,
               aes(x = 0.5, xend = Financial_Rank,
                   y = League_Position, yend = League_Position),
               colour = "#F39C12", linetype = "dashed",
               linewidth = 0.55, alpha = 0.6) +
  geom_segment(data = cross_other,
               aes(x = Financial_Rank, xend = Financial_Rank,
                   y = League_Position, yend = 21.5),
               colour = "#F39C12", linetype = "dashed",
               linewidth = 0.55, alpha = 0.6) +
  # Crosshairs — Leicester (thicker/darker to emphasise most extreme case)
  geom_segment(data = cross_leic,
               aes(x = 0.5, xend = Financial_Rank,
                   y = League_Position, yend = League_Position),
               colour = "#D35400", linetype = "dashed",
  ...
  annotate("text",
           x      = fr_labels$Financial_Rank,
           y      = 21.5,
           label  = paste0("FR ", fr_labels$Financial_Rank),
           colour = "#E67E22", size = 2.7, fontface = "bold",
           hjust  = 0.5, vjust = 1) +
  labs(
