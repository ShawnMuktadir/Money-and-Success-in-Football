...
model_fit <- tibble(
  Model   = c("A", "B", "C", "D", "E", "F"),
  Type    = c("OLS", "OLS", "OLS", "OLS", "Logit", "Logit"),
  Formula = c(
    "Pos ~ Financial_Rank",
    "Pos ~ Financial_Rank + League",
    "Pos ~ Log_Normalized_Value",
    "Pos ~ Log_Norm_Value + League + Season_Index",
    "Champion ~ Financial_Rank",
    "Champion ~ Log_Norm_Value + League"
  ),
  N             = nrow(model_df),
  R2_McFadden   = c(
    round(sumA$r.squared, 4),
    round(sumB$r.squared, 4),
    round(sumC$r.squared, 4),
    round(sumD$r.squared, 4),
    round(1 - modE$deviance / modE$null.deviance, 4),
    round(1 - modF$deviance / modF$null.deviance, 4)
  ),
  Adj_R2        = c(
    round(sumA$adj.r.squared, 4),
    round(sumB$adj.r.squared, 4),
    round(sumC$adj.r.squared, 4),
    round(sumD$adj.r.squared, 4),
    NA_real_, NA_real_
  ),
